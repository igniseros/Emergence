extends EvolvingLifeDot
class_name HerbavoreEvolvingLifeDot


func _init():
	randomize()
	#set name
	name = "Herbavore Evolving Life"
	team = "herbivors"
	#set clors
	color_one = Color(0,0,0)
	color_two = Color(0,0,0)
	color_three = Color(0,0,0)
	
	energy = reproduction_cost
	efficency = 1.5
	
	possible_step_list = [StepEat,StepRandomWalk,StepRandomJump,StepWait]
	behavior = Behavior.new()
	behavior.steps.append_array([StepEat.new(),StepRandomWalk.new(),StepEat.new(),StepRandomWalk.new(),StepEat.new(),StepRandomWalk.new()])
	
	
	reproduction_energy_thresh = 2
	reproduction_cost = 3
	
