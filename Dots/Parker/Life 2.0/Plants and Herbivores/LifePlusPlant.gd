...
var poison_plants_seen_recently = Pile.new(.1)
...
func _init(_is_child = false, _parent = null).(_is_child, _parent):
	...
	energy = FloatAttribute.new("Energy", 8, 0, MAX_ENERGY)
	#--variables--
	basel_metabolic_rate.set_value(.25)
	reproduction_threshold = MutableFloatAttribute.new("Reproducton threshold", 20, .5)
	reproduction_gift.set_value(.5)
	reproduction_chance.set_value(.01)
	...
	allowed_actions = [RandomWalkAction, SpecificWalkAction, PlantGainMoreEnergyAction, PlantShareEnergyAction, 
